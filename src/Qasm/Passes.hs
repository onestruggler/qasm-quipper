...
abstractStmt ln (QasmGateStmt expr) =
    case exprToGate expr of
        Left (n, gate) -> case validateGate gate of
            Nothing  -> Left (replicate n (AstGateStmt gate))
...
-- Converts a list of statements into an AST. If the conversion fails, then a
-- then an appropriate abstraction error is returned.
toAst :: Int -> [Stmt] -> Either [AstStmt] AbstractionErr
toAst ln []           = Left []
toAst ln (stmt:stmts) =
    case abstractStmt ln stmt of
        Left prefix -> case toAst (ln + 1) stmts of
            Left ast  -> Left (prefix ++ ast)
            Right err -> Right err
        Right err -> Right err
